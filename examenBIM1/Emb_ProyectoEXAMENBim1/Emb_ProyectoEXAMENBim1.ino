...
int v=0;
String m;
float matriz_s[4][5]={{470,247,351,2,1}, 
                      {629,443,4,28,2}, 
                      {534,189,340,14,3}, 
                      {655,547,1,1,4}};
float test[1][5]={469,77,334,9,3};//3  dato nuevo para comprobar
void setup() {
 Serial.begin(9600);
//--------------codigo completo 
...
for(;fil_s<4;fil_s++){
     for(;col_s<5;col_s++){
      ...
      if(matriz_s[fil_s][col_s]>100){
         EEPROM.write(conteep,((int)(matriz_s[fil_s][col_s]))/6);
         // Serial.println(EEPROM.read(conteep)/2);
         conteep++;
        
        }else{
           EEPROM.write(conteep,((int)(matriz_s[fil_s][col_s])));
         // Serial.println(EEPROM.read(conteep)/2);
         conteep++;
          }
...
for(;fil_s<4;fil_s++){
     for(;col_s<5;col_s++){
        ...
        } if(matriz_s[fil_s][col_s]>108){
        Serial.print(((float)(EEPROM.read(conteep)*6)));
        Serial.print(','); 
         matriz[fil_s][col_s]=((float)(EEPROM.read(conteep)*6));
        conteep++;
        }
       // conteep++;
       if(matriz_s[fil_s][col_s]<108){
        Serial.print(((float)(EEPROM.read(conteep))));
        Serial.print(','); 
         matriz[fil_s][col_s]=((float)(EEPROM.read(conteep)));
        conteep++;
        }
       // conteep++;
      }
